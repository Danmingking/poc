...
class asset:
	...
	def delayy(self, a):
		for sleep in range(int(a), 0, -1):
			console.print(f' [bold green]#[bold white] Tunggu Sebentar {sleep}{" "*10}', end='\r')
			time.sleep(1)
...
class AccountCreator:
	...
	def banner(self):
		console.print(f"""
[bold grey37]___________ [italic bold sky_blue1]Code By Fajarky[/][bold grey37] ___________           .__          
\_   _____/____    ____  ___\__    ___/___   ____ |  |   ______
 |    __) \__  \ _/ ___\/ __ \|    | /  _ \ /  _ \|  |  /  ___/
 |      \   / __ \\  \__\  ___/|    |(  <_> |  <_> )  |__\___ \ 
[bold cyan3] \___  /  (____  /\___  >___  >____| \____/ \____/|____/____  >
     \/        \/     \/    \/    [italic bold sky_blue1]Facebook creator v1.0[/][bold cyan3]     \/[bold white]
		""")
 ...
 [bold green]2[bold white]) Komentar Otomatis
 [bold green]3[bold white]) Reaksi Otomatis 
 [bold green]4[bold white]) Cek Akun
 [bold green]5[bold white]) exit
		""")
		userInput = console.input(" [bold green]#[bold white] pilih 1-5: ")
  ...
  Tanggal Lahir: {tanggal_lahir}""")
	...
	def looping(self, delay):
		for sleep in range(int(delay), 0, -1):
			console.print(f' [green]#[white] Tunggu: {sleep} success: [green]{Ok}[white] failed: [red]{Fail}[white] check: [yellow]{Cp}[white] ', end='\r')
			time.sleep(1)
	...
	def create_account(self):
		...
		with requests.Session() as r:
			...
			if "session_key" in mek or 'access_token' in mek:
				...
				console.print(f" [bold red]# Kami Tidak Bisa Membuat Akun untuk Anda{' '*10}\r")
...
{{
  ...
  {M}details{P}: {{
    ...
    {M}birthday{P}: {H}{self.tgl_lahir}{P}
				...
				file_path = 'Results/FBNewAkun.json'
		...
		try:
			with requests.Session() as r:
				...
				if 'profilePhoto' in post.text:
					return f"{H}Berhasil memperbarui foto profil{P}"
				else:
					return f"{M}Gagal memperbarui foto profil{P}"
